...
export function Legend() {
  ...
  return {
    draw: (model, handlebarsTemplate, refresh) => {
      ...
      const isPopulationColumnVisible =
        model.getToggle("togglePopulationColumnVisibility") === "visible";
      ...
      Handlebars.registerHelper(
        "color",
        (country) => 'style="color:' + color(country) + ';"'
      );
      ...
      Handlebars.registerHelper(
        ...
        () =>
          'style="display:' +
          (isPopulationColumnVisible ? "table-cell" : "none") +
          '"'
      );
      const legendHeadAndBody = handlebarsTemplate(
        model.getCountriesHolder().getAsArray()
      );
      ...
      function feedbackSelectedCountry(selectedCountry, doNotScrollIntoView) {
        ...
        if (selectedCountry) {
          ...
          $('#chart .category[name="' + selectedCountry + '"]').removeClass(
            "inactive"
          );
          $('#chart .category[name="' + selectedCountry + '"]').addClass(
            "active"
          );
      ...
      const $filterToggle = $('[type="button"]#filterToggle');
      {
        ...
        if (
          countries.length <= 10 &&
          model.getToggle("toggleFilter") === "off"
        ) {
      ...
      $("#legend table.table tbody").on("click", "td.country", (e) => {
        ...
        const selectedCountry = model
          .getCountriesHolder()
          .toggleSelectedCountry($(e.currentTarget).attr("name"));
